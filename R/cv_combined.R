...
#'
#' Combines leave-one-environment-out with K-fold genotype cross-validation.
#' For each dropped environment, recalculates reaction norms and then performs
#' genotype CV with GBLUP (rrBLUP) or BayesB (BGLR).
#'
...
#' head(cv_result)
#' }
cv_combined <- function(gFold, gIteration, SNPs, env_mean_trait,
                        exp_trait,
  ...
  total_steps <- n_envs * gIteration * gFold
  step <- 0L
  results <- list()

  for (e_i in seq_len(n_envs)) {
    ...
    block_idx <- floor(seq(1, nrow(ab_df), length = gFold + 1))
    if (block_idx[length(block_idx)] < nrow(ab_df)) {
      block_idx[length(block_idx)] <- nrow(ab_df)
    }

    for (n in seq_len(gIteration)) {
      env_idx <- sample(seq_len(nrow(ab_df)), nrow(ab_df))
